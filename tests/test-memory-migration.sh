...
entries = [
    ...
    {'id': 2, 'key': 'news-upgrade', 'content': 'project-a needs new feed parser', 'timestamp': '2026-01-02T00:00:00Z'},
...
entries = [
    ...
    {'id': 2, 'key': 'feed-parser', 'content': 'project-a news feed', 'timestamp': '2026-01-02T00:00:00Z'},
    {'id': 3, 'key': 'report-gen', 'content': 'project-b CDW section', 'timestamp': '2026-01-03T00:00:00Z'}
]
with open('$MEMORY_DIR/legacy.json', 'w') as f:
    json.dump(entries, f)
"
  local output
  output=$(bash "$PROJECT_ROOT/scripts/aegis-migrate-memory.sh" --dry-run 2>&1)
  local aegis_match project-a_match radiant_match
  aegis_match=$(echo "$output" | grep -c "aegis" || true)
  project-a_match=$(echo "$output" | grep -c "project-a" || true)
  radiant_match=$(echo "$output" | grep -c "project-b" || true)
  if [[ "$aegis_match" -ge 1 && "$project-a_match" -ge 1 && "$radiant_match" -ge 1 ]]; then
...
entries = [
    ...
    {'id': 2, 'key': 'feed', 'content': 'project-a feed update', 'timestamp': '2026-01-02T00:00:00Z'}
]
with open('$MEMORY_DIR/legacy.json', 'w') as f:
    json.dump(entries, f)
"
  bash "$PROJECT_ROOT/scripts/aegis-migrate-memory.sh" --auto > /dev/null 2>&1
  if [[ -f "$MEMORY_DIR/aegis-project.json" && -f "$MEMORY_DIR/project-a-project.json" ]]; then
    pass "[MEM-05] auto mode creates project-scoped files"
  else
    fail "[MEM-05] auto mode writes" "expected aegis-project.json and project-a-project.json"
  fi
  teardown
}
